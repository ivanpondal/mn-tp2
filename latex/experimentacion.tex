...
\begin{center}
    ...
    \addplot[color=green] table[x=nodos,y=esp]{datos/pr-tiempos-esparsa.out};
    \addplot[color=red] table[x=nodos,y=noesp]{datos/pr-tiempos-esparsa.out};
    \legend{Esparsa, No esparsa}
    \end{axis}
    \end{tikzpicture}
\end{center}
...
ranking es ver cómo evolucionan a lo largo del tiempo ya que es importanto
analizar los casos donde se producen cambios abruptos como para que no exista
ninguna anomalía en el resultado final. Es por esto que en el siguiente
experimento lo que hicimos es tomar los dos modelos descritos y estudiar
en paralelo su evolución a lo largo del tiempo.

Para esto tomamos los que terminaron en los primeros 8 lugares según el modelo
...
ambos modelos. Cabe destacar que para el sistema de puntaje de la AFA como este
no trabaja asignando a cada equipo un peso cuya suma total da 1, para poder
comparar los dos métodos, para cada fecha tomamos como puntaje el porcentaje del
total de puntos para esa fecha en particular.
